<script lang="ts">
  ...
  $effect(() => {
    ...
    if (a.loaded && a.me === null) {
      goto('/sign-in?next=' + encodeURIComponent('/competitions/' + id));
    }
  });
...
<main>
  <p><a href="/competitions">← Back to competitions</a></p>
  ...
  {:else if comp}
    <header>
      ...
      <p class="quick-links">
        <a href={`/competitions/${comp.id}/problems`}>Problems editor →</a>
        <a href={`/competitions/${comp.id}/registrations`}>Registrations →</a>
