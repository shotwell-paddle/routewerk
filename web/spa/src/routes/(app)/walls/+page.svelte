...
<div class="page">
  <header class="page-header">
    ...
    {#if locId && canManage}
      <a class="btn-primary" href="/walls/new">+ New wall</a>
    {/if}
  </header>
  ...
  {:else if walls.length === 0}
    <div class="empty-card">
      ...
      {#if canManage}
        ...
        <a class="btn-primary" href="/walls/new">Create wall</a>
      {:else}
    ...
    {#if canSeeArchived && archivedWalls.length > 0}
      ...
      <div class="grid archived-grid">
        {#each archivedWalls as w (w.id)}
          <a class="card archived" href="/walls/{w.id}">
            <div class="card-head">
              <span class="type-badge type-{w.wall_type}">{w.wall_type}</span>
              <h3 class="name">{w.name}</h3>
            </div>
