<script lang="ts">
  ...
  // up to now /app/settings/progressions linked out to HTMX for create.
  ...
  async function submit(body: Partial<QuestShape>) {
    ...
    try {
      ...
      goto(`/app/settings/progressions/quests/${created.id}/edit`);
    } catch (err) {
...
<div class="page">
  <p><a class="back" href="/app/settings/progressions">← Progressions admin</a></p>
  <h1>New quest</h1>
  ...
  {:else}
    <QuestForm
      ...
      onSubmit={submit}
      onCancel={() => goto('/app/settings/progressions')}
      {saving}
      {error} />
  {/if}
</div>
